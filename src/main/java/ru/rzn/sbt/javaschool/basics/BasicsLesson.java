...
public class BasicsLesson {
    ...
    public Object newClass() {
        Object o = new LethalWeapon("s",1.2, 3);
     ...
     */
    public void serial() {
        LethalWeapon obj = new LethalWeapon("s",1.2, 3);
        obj.getSerial();
    }
    ...
    public String stringExercise1(String input) {
        String result;
        int i = input.length();
        if (input.indexOf("S")+1< i) {
            result = (input.substring(2, input.indexOf("S")+1)).toUpperCase();
        }
        else {
            result = (input.substring(2,  input.lastIndexOf(input))).toUpperCase();
        }
        return result;
    ...
    public int stringExercise2(String input) {
        ...
        int i = 0, countСomma = 0, countHyphen = 0, countSpace=0;
        while ( i  <=inputNew.length()-1) {
            if (inputNew.charAt(i) == ',') {
                countСomma++;
            }
            if (inputNew.charAt(i) == '-') {
                countHyphen++;
    ...
    public String stringExercise3(String input) {
        ...
        if (countWords == 3) {
            if (input.contains("Hello")) {
                inputNew = inputNew.substring(0, inputNew.indexOf(" ")+1) +
                        inputNew.substring(inputNew.lastIndexOf(" ")+1, inputNew.length());
            }
            if (input.contains("Goodbye")) {
                inputNew = inputNew.substring(0, inputNew.indexOf(" ")+1) +
                        inputNew.substring(inputNew.lastIndexOf(" ")+1, inputNew.length());
                if (inputNew.lastIndexOf("!") != 0 && inputNew.lastIndexOf("!") == inputNew.length()-1) {
                    inputNew = inputNew.substring(0, inputNew.length()-1);
                } else {
                    inputNew = inputNew.concat("!");
                }
            }
        }
        if (countWords == 2) {
            if (input.contains("Hello")) {
                int above, bellow, middle;
                char symbol;
                StringBuilder inputBuild = new StringBuilder(inputNew);
                bellow = 0;
                middle=inputNew.length()/2;
                above=inputNew.length()-1;
                for (int i = 0; i < middle; i++, bellow++, above--) {
                    symbol = inputNew.charAt(bellow);
                    inputBuild.setCharAt(bellow, inputNew.charAt(above));
                    inputBuild.setCharAt(above, symbol);
                }
                inputNew = inputBuild.toString();
            }
            if (input.contains("Goodbye")) {
                String inputNewFist = inputNew.substring(0, inputNew.indexOf(" "));

                String inputNewSecond = inputNew.substring(inputNew.lastIndexOf(" ")+1, inputNew.length());
                inputNew = (inputNewSecond+" "+inputNewFist).toLowerCase();
                }
            }
